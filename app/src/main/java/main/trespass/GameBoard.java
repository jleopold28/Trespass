...
public class GameBoard {
    ...
    *         valid tile, and the second index is the column of the valid tile
    ...
    public ArrayList<int[]> getValidTiles(int row, int col) {
        ...
        slideValidation(row,col); //get the valid tiles when moving left or right
    ...
    private void forwardValidation(int row, int col) {
        if (row>0) {
            if ((gameBoard[row-1][col].isBlank()) && (!validTiles.contains(boardPos[row-1][col])) ) { //check if the front tile is blank
                ...
                slideValidation(row-1,col); // get the slide moving valid tiles after moving forward
                jumpValidation(row-1,col); // get the jumping valid tiles after moving forward
            }
        }
    }
    private void slideValidation(int row, int col) {
        if (col>0) {
            if ((gameBoard[row][col-1].isBlank()) && (!validTiles.contains(boardPos[row][col-1]))) { //check if the left tile is blank
                validTiles.add(boardPos[row][col-1]); //the left tile is valid
                slideValidation(row,col-1); //get the slide moving valid tiles after moving to the left
                jumpValidation(row,col-1); //get the jumping valid tiles after moving to the left
            }
        }
        if (col<colCount-1) {
            if ((gameBoard[row][col+1].isBlank()) && (!validTiles.contains(boardPos[row][col+1]))) { //check if the right tile is blank
                validTiles.add(boardPos[row][col+1]); //the right tile is valid
                slideValidation(row,col+1); //get the slide moving valid tiles after moving to the right
